...
(function($){
    ...
    var fsWrapper = '<div class="variable000001" onclick="$.failsafe.putToTop();"></div><div class="fs-container" onclick="$.failsafe.putToTop();"><div class="fs-fig"></div></div>';
    var netFig = '<i class="texto-grande material-icons text-danger">signal_wifi_off</i>';
    var batteryFig = ' ';
    ...
    $.extend ($.failsafe,{
            ...
            if(netFlag == "online") {
                ...
                if(batteryFlag == "batteryLow") {
                    ...
                    $.failsafe.init(function(){
                        ...
                        $('#intentandoxxddd').text("Intentando establecer conexicon con "+siteName);
                        $('.fs-container').addClass("animacion005");
                        $('.variable000001').addClass("animacion004");
        ...
        onOnline : function(opts) {
            if(netFlag == "offline") {
                ...
                if($('.variable000001').length != 0 || $('.fs-container').length !=0)
        ...
        putToTop : function() {
            $('.fs-fig').remove();
            $('.variable000001').animate({height:'60px'},1000);
            $('.fs-container').animate({marginTop:'0'},1000);
            $('.fs-container').fadeOut('100').remove();  
            $('.variable000001').fadeOut('100').remove();  
            toastr.options.timeOut=0,toastr.error('the connection is weak');
        },
                
        // removes all tags added by failsafe
        remove : function(opts) {
            $('.variable000001').fadeOut(opts.removeDelay).remove();
            $('.fs-container').fadeOut(opts.removeDelay).remove();
            toastr.clear();
            console.clear();
            toastr.options.timeOut=1500;
            toastr.success('Connection Recovered');
        },
        ...
        defaults : {
            ...
            offlineMsg:"the connection is weak ",
            ...
            $(function(){
                $.failsafe({checkUrl:" ",checkInterval:30000,disableElements:".btn"});
            });